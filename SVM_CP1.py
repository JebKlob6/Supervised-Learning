...
3. Systematic testing of KNN parameters [2,5,7,9,11]
...
def create_output_directories():
    ...
    directories = ["figures", "figures/svmcp1", "results", "data_analysis"]
...
def load_and_prepare():
    ...
    selected_features = [
        ...
        "Smoking", "Target_Severity_Score", "Treatment_Cost_USD", "Age"
    ]
    ...
    data['Cancer_Stage_Binary'] = data[target].apply(
    ...
    # Prepare X and y
    y = data["Cancer_Stage_Binary"].copy()
...
def svm_learning_curve(X_train, X_test, y_train, y_test, kernel, C, gamma=None, degree=None):
    ...
    plt.legend(loc='best')
    plt.grid(True)
    os.makedirs("figures/svmcp1", exist_ok=True)
    plt.savefig(f'figures/svmcp1/learning_curve_{kernel}.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.grid(True)
    os.makedirs("figures/svmcp1", exist_ok=True)
    plt.savefig(f'figures/svmcp1/time_curve_{kernel}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def svm_validation_curve(X_train, y_train, kernel, param_name, param_range, C=1.0, gamma='scale', degree=3):
    ...
    if kernel == 'linear':
        ...
        base_estimator = SVC(kernel='poly', class_weight='balanced', random_state=RANDOM_STATE, degree=degree,
                             gamma=gamma, C=C)
    else:
    ...
    plt.grid(alpha=0.3)
    os.makedirs("figures/svmcp1", exist_ok=True)
    plt.savefig(f"figures/svmcp1/validation_curve_{kernel}_{param_name}.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    y_train_sub = y_train[sub_idx].copy()
    ...
    svm_validation_curve(X_train_sub, y_train_sub, kernel='poly', param_name='C', param_range=C_range, gamma=0.1,
                         degree=2)
    svm_validation_curve(X_train_sub, y_train_sub, kernel='poly', param_name='degree', param_range=degree_range, C=1,
                         gamma=0.1)

    print("\nSVM experiment complete. See figures/svmcp1/ for plots.")
