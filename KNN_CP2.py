"""
Enhanced KNN Analysis for Company Bankruptcy Dataset
===================================================
...
4. K-fold validation testing [1, 3, 5]
...
def create_output_directories():
    ...
    directories = ["figures", "figures/knncp2", "results", "data_analysis"]
...
def load_and_prepare():
    ...
    print("\nClass distribution:")
    print(pd.Series(y_encoded).value_counts(normalize=True).round(3))
...
def create_comprehensive_visualizations(results_df):
    ...
    os.makedirs('figures/knncp2', exist_ok=True)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('figures/knncp2/knn_performance_metrics.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('figures/knncp2/knn_train_test_accuracy.png')
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('figures/knncp2/knn_cv_scores.png')
    plt.close()
...
def create_final_model_evaluation(X_train, X_test, y_train, y_test, best_params, label_encoder):
    ...
    plt.tight_layout()
    plt.savefig('figures/knncp2/final_confusion_matrix.png', dpi=300, bbox_inches='tight')
...
def generate_learning_curves(X_train, X_test, y_train, y_test, best_params):
    ...
    plt.legend(loc='best')
    plt.grid(True)
    plt.savefig('figures/knncp2/knn_learning_curves.png')
    plt.close()
